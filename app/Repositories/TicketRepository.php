    ...
    public function webStore($input)
    {
        try {
            ...
            if(getLoggedInUserId() != getAdminUserId()){
                ...
                $message = 'Hi '.ucfirst(getLoggedInUser()->name).", 
                We\’ve received your IT support request (Ticket #".$input['ticket_id'].") and are working on it! Here\’s a quick summary: 
                • Issue: ".$input['title']." 
                • Submitted On: ".date("d-m-Y h:i:s")." 
                • Priority: High 
                
                Our team will get back to you soon with a solution. 
                If you have any updates or additional info, just reply to this email. 
                
                Thanks, MDLBEAST Venues IT";
                $notificationRecord2 = [
                    'We\’ve Got Your Request! #'.$input['ticket_id'],
                    UserNotification::NEW_TICKET_CREATED,
                    $message,
                    Auth::id()
                ];
                addNotification($notificationRecord2);
            ...
            sendEmailToCustomer($input['created_by'],
                ...
                'Ticket Successfully Created',
                $input);
                if(isset($input['company_id']) && $input['company_id'] == 3){
                    $ticket->assignTo()->sync(11);
                }else{
                    $ticket->assignTo()->sync(10);
                }
