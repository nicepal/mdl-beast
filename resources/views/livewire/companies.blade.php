        ...
        <div class="col-lg-12 col-md-12">
            <div class="row mb-3 justify-content-end flex-wrap px-mobile-3">
                <div class="width-mobile-100">
                    <div class="selectgroup">
                        <input wire:model.debounce.100ms="searchByCaompany" type="search"
                               id="searchByCategory"
                               autocomplete="off"
                               placeholder="{{ __('messages.company.search_company') }}"
                        ...
                        <div class="row paginatorRow justify-content-between">
                            <div class="px-3">
                                <span class="d-inline-flex">
                                    ...
                                    <span class="font-weight-bold ml-1 mr-1">{{ $companies->firstItem() }}</span> -
                                    <span class="font-weight-bold ml-1 mr-1">{{ $companies->lastItem() }}</span> {{ __('messages.common.of') }}
                                    <span class="font-weight-bold ml-1">{{ $companies->total() }}</span>
                                </span>
                            </div>
            ...
            @else
                <div class="col-lg-12 col-md-12 d-flex justify-content-center">
                    @if($searchByCompany == null || empty($searchByCompany))
                        <h1 class="font-size">{{ __('messages.company.companies_not_available') }}</h1>
                    @else
                        <h1 class="font-size">{{ __('messages.company.companies_no_found') }}</h1>
                    @endif
                </div>
            @endif
        </div>
